...
if Reinforce_mode:
    ...
    dataset = 'chem_round1_best'
    vocab = getVocab(vocab_path['General'])
    if 'pair' in dataset:
        train_text = getPairTextList(text_path[dataset])
        train_dataset = pairDataset(train_text, vocab, block_size)
    else:
        train_text = getTextList(text_path[dataset])
        train_dataset = polypeptideDataset(train_text, vocab, block_size)
    test_dataset = None

elif train_conf.mode == 'pretrain' or 'pair' not in dataset:
    mask = True
    text = getTextList(text_path[dataset])
    ...
    mask = True
    text = getPairTextList(text_path[dataset])
    train_text, test_text = divideData(text, 0.8)
    vocab = getVocab(vocab_path['General'])
...
train_conf.alterAttr(device='cuda:0',max_epochs=350, batch_size=256, learning_rate=7e-4, weight_decay=1e-4,lr_decay=True, doSave=True, doTest=True,
                        ...
                        mask=mask, mask_ratio=0.3, max_pred=20, num_workers=5, dataset=dataset,warmup_epochs=30, protein_path=text_path, seed=seed)


sample_conf = makeSampleConfig(
    dataset='antifungal',isPair=True, isRandom=False, sample_times=50, batch_size=10, protein_path=text_path,
    mask=mask, mask_ratio=0.3, max_pred=20, num_workers=5,)
sample_dataset = polypeptideDataset(getTextList(text_path[sample_conf.dataset]), vocab, block_size)
...
######################################################## finetune ####################################################################
if train_conf.mode == 'finetune' and 'chem' not in dataset:
    mask = True
    train_conf.alterAttr(
                        pretrained_model = 'output/pretrain/dataset=uniport_all_60,seed=2,optimizer=Ranger,batch_size=256,epochs=350,lr=0.0007,weight_decay=0.0001,lr_decay=True,lr_scheduler=WarmupLR,warmupEpochs=30/epoch_349/model.pkl',
                        device='cuda:0',max_epochs=200, batch_size=128, learning_rate=6e-5, weight_decay=1e-3,lr_decay=True, doSave=True, doTest=True,
                        model=model, train_dataset=train_dataset, test_dataset= test_dataset, optimizer='Ranger',lr_scheduler='WarmupLR', #{'WarmupLR','CyclicLR'}
                        mask=mask, mask_ratio=0.3, max_pred=20, num_workers=5, dataset=dataset,warmup_epochs=15, protein_path=text_path, seed=seed)
    ...
    sample_conf = makeSampleConfig(
        dataset=f'inter_test_{reinforce_conf.optimize_amp}_all',isPair=False, isRandom=False, sample_times=20, batch_size=5, protein_path=text_path,
...
######################################################## re-finetune ####################################################################
if 'chem' in dataset and not Reinforce_mode:
    mask = False
    train_conf.alterAttr(
                        pretrained_model = f'output/finetune/dataset=uniport_all_60,seed=2,optimizer=Ranger,batch_size=256,epochs=350,lr=0.0007,weight_decay=0.0001,lr_decay=True,lr_scheduler=WarmupLR,warmupEpochs=30_epoch_349/dataset=random_pair,seed={seed},optimizer=Ranger,batch_size=128,epochs=200,lr=6e-05,weight_decay=0.001,lr_decay=True,lr_scheduler=WarmupLR,warmupEpochs=15/epoch_199/model.pkl', # general model
                        # pretrained_model = 'output/finetune/dataset=random_pair,seed=6,optimizer=Ranger,batch_size=128,epochs=200,lr=6e-05,weight_decay=0.001,lr_decay=True,lr_scheduler=WarmupLR,warmupEpochs=15_epoch_199/dataset=random_chem_penetritin_high_activity_all_pair,seed=6,optimizer=Ranger,batch_size=32,epochs=200,lr=1e-05,weight_decay=0.001,lr_decay=True,lr_scheduler=WarmupLR,warmupEpochs=15/epoch_199/model.pkl', #round 1 model
    ...
    text_path[f'inter_test_{reinforce_conf.optimize_amp}_all']=  f'{train_conf.basicPath}reinforcement_proteins_package/{reinforce_conf.optimize_amp}_num_10.txt'
    sample_conf = makeSampleConfig(
        dataset='optimize_amp',isPair=False, isRandom=False, sample_times=50, batch_size=5, protein_path=text_path,
        mask=mask, mask_ratio=0.3, max_pred=4, num_workers=5,outside_record=False)
    sample_dataset = polypeptideDataset(getTextList(text_path[sample_conf.dataset]), vocab, block_size)
    sample_conf.alterAttr(dataset=sample_dataset)
######################################################## re-finetune ####################################################################
